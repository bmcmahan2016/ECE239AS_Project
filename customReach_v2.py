...
# define some values
NUM_EPISODES = 4005        # number of episodes we will train on
lr = 1e-3               # learning rate for Q-learning
discountFactor = 0.95    # discount factor
delta = 1   # single-dimension standard deviation of additive action noise
epsilon = 0.1
actionScale = 1
scaleFactor = 1

# create the environment
env = customEnv.env(m=1, dt=0.022)
...
value = multiLayer(observationDim, [10], 1)
...
#policyOptimizer = optim.SGD(policy.parameters(), lr=0.01, weight_decay=0)
policyOptimizer = optim.Adam(policy.parameters(), lr=0.01)
...
for currEpisodeNum in range(NUM_EPISODES):
    ...
    #startTimeStep = np.random.randint(0, MAX_TIME_STEPS)
    startTimeStep = 0
    obs = env.reset(cursorPos[refInd, startTimeStep], refVel[refInd, startTimeStep])
    targetPos = env.setTarget(refTargets[refInd])
    ...
    for currTimeStep in range(startTimeStep, MAX_TIME_STEPS):
        ...
        Vel[currTimeStep] = vel
        direction = targetPos - x
        direction /= np.linalg.norm(direction)
        #reward = np.dot(vel, direction)/np.linalg.norm(vel)
        taskreward = np.exp(-5*(1 - np.dot(vel, direction)/np.linalg.norm(vel))) + np.exp(-10*(np.linalg.norm(targetPos - x))**2)
