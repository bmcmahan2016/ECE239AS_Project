...
def probOfAction(action, state, policy):
    ...
    # formats data to appropriate shape for GaussianProbability
    meanAction = policy(state).view(4,1)
    action = action.view(4,1)
    cov = 0.1     # this should be cleaned
    
    # computes probability of playing action under current policy
    actionProbability = GaussianProbability(action, meanAction, cov)
    return actionProbability

# define some values
NUM_EPISODES = 3000        # number of episodes we will train on
lr = 1e-2                  # learning rate for Q-learning
...
for currEpisodeNum in range(NUM_EPISODES):
    obs = env.reset()
    env.env.distance_threshold = 0.5
    ...
    action = torch.from_numpy(actionHist).t()
    actionMean = policy(state).t()
    errorSum = GaussianProbability(action, actionMean, 0.1)

    policyOptimizer.zero_grad()
    advantage = (TD_target - valueEstimate).detach()
    meanError = errorSum @ advantage.double() #advantage*torch.sum(errorSum)
    meanError.backward()
